...
`ai.EmbeddingDocuments` and persisted `ai.EmbeddingChunks` from
`customer.ProductReviews` joined to `catalog.Products`) and then
`local/01-feature-detection.sql`.
...
At compatibility level 170 or later, `AI_GENERATE_CHUNKS` creates fixed-size
chunks and retains source product/review provenance, order, offset, length, and
chunk-set identifiers. Below that compatibility level the script reports a
truthful skip rather than claiming that chunk generation ran.
