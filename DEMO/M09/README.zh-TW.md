...
本機指令碼會記錄實際的引擎／版本，並測試是否可使用 `vector` 型別。它會偵測外部模型或模型目錄及端點設定是否可用；除非引擎與端點設定支援，否則會將外部模型與內嵌工作回報為已略過。`azure/01-external-model.sql` 是受控識別範本（`AdventureGearEmbeddingModel`）；請在執行時提供 SQLCMD 變數及已核准的端點，絕不可認可金鑰、權杖、租用戶 ID 或物件 ID。
