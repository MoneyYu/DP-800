...
resource "azurerm_virtual_network" "lab01h" {
  count = var.enable_sql_vm_2022 ? 1 : 0
...
resource "azurerm_subnet" "lab01h" {
  ...
  name                 = "default"
  resource_group_name  = azurerm_resource_group.dp300.name
  virtual_network_name = azurerm_virtual_network.lab01h[0].name
...
resource "azurerm_public_ip" "lab01h" {
  ...
  name                = "${local.lab01h_name}-${var.group_postfix}-pip-${local.random_str}"
  location            = azurerm_resource_group.dp300.location
  resource_group_name = azurerm_resource_group.dp300.name
  allocation_method   = "Static"
  domain_name_label   = "${local.lab01h_name}-${var.group_postfix}-pip-${local.random_str}"
  sku                 = "Standard"
...
resource "azurerm_network_security_rule" "lab01h01" {
  ...
  resource_group_name         = azurerm_resource_group.dp300.name
  network_security_group_name = azurerm_network_security_group.lab01h[0].name
}
...
resource "azurerm_network_interface" "lab01h" {
  ...
  ip_configuration {
    ...
    subnet_id                     = azurerm_subnet.lab01h[0].id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.lab01h[0].id
  }
...
resource "azurerm_network_interface_security_group_association" "lab01h" {
  ...
  network_interface_id      = azurerm_network_interface.lab01h[0].id
  network_security_group_id = azurerm_network_security_group.lab01h[0].id
}
...
resource "azurerm_windows_virtual_machine" "lab01h" {
  ...
  location              = azurerm_resource_group.dp300.location
  resource_group_name   = azurerm_resource_group.dp300.name
  network_interface_ids = [azurerm_network_interface.lab01h[0].id]
  size                  = "Standard_B2s"
...
resource "azurerm_virtual_machine_data_disk_attachment" "lab01h_datadisk_attach" {
  ...
  managed_disk_id    = azurerm_managed_disk.lab01h_datadisk[0].id
  virtual_machine_id = azurerm_windows_virtual_machine.lab01h[0].id
  lun                = 1
  caching            = "ReadWrite"
}
...
resource "azurerm_virtual_machine_data_disk_attachment" "lab01h_logdisk_attach" {
  ...
  managed_disk_id    = azurerm_managed_disk.lab01h_logdisk[0].id
  virtual_machine_id = azurerm_windows_virtual_machine.lab01h[0].id
  lun                = 2
  caching            = "ReadWrite"
}
...
resource "azurerm_mssql_virtual_machine" "lab01h" {
  ...
  storage_configuration {
    ...
    data_settings {
      ...
      luns              = [azurerm_virtual_machine_data_disk_attachment.lab01h_datadisk_attach[0].lun]
    }

    log_settings {
      default_file_path = "G:\\log"
      luns              = [azurerm_virtual_machine_data_disk_attachment.lab01h_logdisk_attach[0].lun]
    }
  }
